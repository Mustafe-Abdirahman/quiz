...
export default function MultiplayerRoom() {
  ...
  const loadMyQuestions = useCallback((r, allQs) => {
    if (!r || !allQs.length) return [];
    const me = r.players.find(p => p.userId === user?.userId);
    if (!me || !me.questionIds?.length) return [];
    return me.questionIds
  ...
  }, [currentQ, myQuestions.length, room, user, addToast, refreshRoom]);

  useEffect(() => {
    if (!room || !allQuestions.length) return;
    if (room.status === 'playing' && !myQuestions.length) {
      const loaded = loadMyQuestions(room, allQuestions);
      if (loaded.length) setMyQuestions(loaded);
    }
  ...
  }, [room, allQuestions, myQuestions.length, loadMyQuestions, finished]);
        ...
        <div className="w-full max-w-md text-center">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Waiting Room</h2>
              <p className="text-gray-500 dark:text-gray-400 mb-4">Room: <span className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{room.code}</span></p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Quiz: {quiz.title}</p>
              <div className="mb-6">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Players ({room.players.length}/{room.maxPlayers || 4}):</p>
                <div className="flex flex-wrap justify-center gap-2">
                  {room.players.map(p => (
                    <span key={p.userId} className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/20 rounded-full text-sm text-indigo-700 dark:text-indigo-300 font-medium">
                      {p.username}{p.userId === user?.userId ? ' (You)' : ''}
                    </span>
                  ))}
                </div>
                <p className="text-xs text-gray-400 mt-3">
                  {allQuestions.length} questions divided among {room.players.length} player(s)
                  &mdash; ~{Math.ceil(allQuestions.length / room.players.length)} each
                </p>
              </div>
              {room.hostId === user?.userId && (
                <button onClick={handleStartGame} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium transition-colors">
                  Start Game
                </button>
              )}
              {room.hostId !== user?.userId && (
        ...
        <div className="w-full max-w-lg">
            ...
            {winner && !isTie && (
              <div className="text-center mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400">Score: {winner.score}</p>
              </div>
            )}

            <div className="space-y-2 mb-6">
              {sorted.map((p, i) => (
                <div key={p.userId} className={`flex items-center gap-3 p-3 rounded-lg ${i === 0 ? 'bg-yellow-50 dark:bg-yellow-900/20 ring-1 ring-yellow-400' : 'bg-gray-50 dark:bg-gray-700/50'}`}>
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{p.username}{p.userId === user?.userId ? ' (You)' : ''}</p>
                    <p className="text-xs text-gray-500">{p.correct} correct &bull; {p.incorrect} incorrect &bull; {p.questionIds?.length || 0} questions</p>
                  </div>
            ...
            <button onClick={() => navigate(roomRoute(role, '/rooms'))} className="w-full px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">
              Back to Rooms
            </button>
          </div>
        </div>
  ...
  const myTotal = me?.questionIds?.length || myQuestions.length || 1;
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h1 className="text-lg font-bold text-gray-900 dark:text-white">Room {room.code}</h1>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Your Question {currentQ + 1} of {myTotal}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mt-4 animate-fadeIn">
              ...
              {answered && (
                <div className="mt-6 animate-fadeIn">
                  <div className={`p-3 rounded-lg text-sm font-medium flex items-center gap-2 ${selectedAnswer === question.correctAnswer ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
                    {selectedAnswer === question.correctAnswer ? <><FiCheck size={16} /> Correct! +10 points</> : <><FiX size={16} /> Incorrect! Answer: {question.options[question.correctAnswer]}</>}
                  </div>
          ...
          <div>
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sticky top-24">
              ...
              <div className="space-y-2">
                {players.map((p, i) => {
                  const pTotal = p.questionIds?.length || 0;
                  const pProgress = p.finished ? pTotal : Math.min(p.currentQuestion || 0, pTotal);
                  return (
                    <div key={p.userId} className={`flex items-center gap-2 p-2 rounded-lg ${p.userId === user?.userId ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-gray-50 dark:bg-gray-700/50'}`}>
