    ...
    <AdminLayout>
      <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button onClick={openRegister} className="self-start sm:self-auto">
            <FiUserPlus size={16} />
            Add User
          </Button>
        </div>
                ...
                {filteredUsers.map(u => (
                  <div key={u.id} className="p-4 flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-xs font-bold text-indigo-600 dark:text-indigo-400 shrink-0">
                        {getInitials(u.username)}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{u.username}</p>
                        {u.fullName && <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{u.fullName}</p>}
